...
class SystemSetup(object):
    ...
    def oracle_setup(self, directory_containing_rpms):
        ...
        self.rpm_files['sqlplus'] = glob.glob('{}/oracle*sqlplus*rpm'.format(
            directory_containing_rpms))[0]
        self.rpm_files['tools'] = glob.glob('{}/oracle*tools*rpm'.format(
            directory_containing_rpms))[0]
        self.rpm_files['jdbc'] = glob.glob('{}/oracle*jdbc*rpm'.format(
            directory_containing_rpms))[0]
        self.rpm_files['odbc'] = glob.glob('{}/oracle*odbc*rpm'.format(
            directory_containing_rpms))[0]
        ...
        subprocess.check_call(['alien', '-iv', self.rpm_files['tools']])
        subprocess.check_call(['alien', '-iv', self.rpm_files['jdbc']])
        subprocess.check_call(['alien', '-iv', self.rpm_files['odbc']])
        ...
        with open('{}/.bashrc'.format(os.environ['HOME']), 'a') as bashrc:
            ...
            bashrc.write('export TNS_ADMIN=/usr/lib/oracle/'
                         '{}/client64/network/admin\n'.format(oracle_version))
            bashrc.write('export ORACLE_PATH_INCLUDES=/usr/include/oracle/'
                         '{}/client64\n'.format(oracle_version))
            bashrc.write('export PATH=$PATH:$ORACLE_HOME/bin\n')
